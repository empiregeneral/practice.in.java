...
public class hands_buzdalov {
    ...
    static class K2_2_1 extends Hand<K2_2_1> {
        ...
        public int compareToImpl(K2_2_1 that) {
            if (r2_1 != that.r2_1) {
                return r2_1 - that.r2_1;
            }
            if (r2_2 != that.r2_2) {
                return r2_2 - that.r2_2;
            }
            return r1 - that.r1;
        }
    }
    static class Pair extends Hand<Pair> {
        ...
        public int compareToImpl(Pair that) {
            ...
            for (int i = 3, j = 4, k = 4; i > 0; --i) {
                while (this.cards[j].rank == p){
                    --j;
                }
                while (that.cards[k].rank == p) {
                    --k;
                }
    ...
    public static void main(String[] args) throws IOException {
        Main.Scanner scanner = new Main.Scanner(System.in);

        int n = Integer.parseInt(scanner.next());
        for (int i = 0; i < n; i++) {
            scanner.nextLine();
            String inputLine = scanner.line;
            Card[] cards = Arrays.stream(inputLine.split(" ")).map(e -> new Card(e)).toArray(Card[]::new);
